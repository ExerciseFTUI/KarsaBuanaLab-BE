...
async function getTotalClientPerYear() {
  try {
    ...
    const clientCount = clientList.reduce(
      (prev, curr) => ((prev[curr] = ++prev[curr] || 1), prev),
      {}
    );
...
async function getOfferPerMonth() {
  try {
    ...
    resultRunning.forEach((project) => {
      if (
        new Date(project.created_at).getFullYear() !== new Date().getFullYear()
      )
        return;
    ...
    resultFinished.forEach((project) => {
      ...
      if (valuation === undefined || valuation === null || valuation === NaN)
        return;
